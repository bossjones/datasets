...
langchain-migrate-diff:
	langchain-cli migrate --include-ipynb --diff datasets

# Perform LangChain migration
langchain-migrate:
	langchain-cli migrate --include-ipynb datasets
...
uv_create_token:
	{{PYTHON}} -c "from datasets.cli import create_token; create_token()"

# Show current database state
uv_db_current:
	{{PYTHON}} -c "from datasets.cli import db_current; db_current()"

# Upgrade database to latest version
uv_db_upgrade:
	{{PYTHON}} -c "from datasets.cli import db_upgrade; db_upgrade()"

# Downgrade database to previous version
uv_db_downgrade:
	{{PYTHON}} -c "from datasets.cli import db_downgrade; db_downgrade()"

# Export a collection of data
uv_export_collection:
	{{PYTHON}} -c "from datasets.cli import export_collection; export_collection()"

# Import a collection of data
uv_import_collection:
	{{PYTHON}} -c "from datasets.cli import import_collection; import_collection()"

# Import a single file
uv_import_file:
	{{PYTHON}} -c "from datasets.cli import import_file; import_file()"
...
uv_pylint_error_only:
	{{UV_RUN}} pylint --output-format=colorized --disable=all --max-line-length=120 --enable=F,E --rcfile pyproject.toml datasets tests

# Run pylint on all files
uv_lint_all:
	{{PYTHON}} -m pylint -j4 --output-format=colorized --rcfile pyproject.toml tests datasets
...
uv_typecheck_verify_types:
	{{UV_RUN}} pyright --verifytypes datasets --ignoreexternal --verbose
...
add-cursor-context:
	mkdir -p datasets/vendored || true
	gh repo clone universityofprofessorex/cerebro-bot datasets/vendored/cerebro-bot || true && cd datasets/vendored/cerebro-bot && git checkout feature-discord-utils && cd ../../..
	gh repo clone bossjones/sandbox_agent datasets/vendored/sandbox_agent || true
	gh repo clone langchain-ai/retrieval-agent-template datasets/vendored/retrieval-agent-template || true
	gh repo clone langchain-ai/rag-research-agent-template datasets/vendored/rag-research-agent-template || true
	gh repo clone langchain-ai/memory-template datasets/vendored/memory-template || true
	gh repo clone langchain-ai/react-agent datasets/vendored/react-agent || true
	gh repo clone langchain-ai/chat-langchain datasets/vendored/chat-langchain || true
	gh repo clone bossjones/goob_ai datasets/vendored/goob_ai || true
	gh repo clone langchain-ai/langchain datasets/vendored/langchain || true
	gh repo clone langchain-ai/langgraph datasets/vendored/langgraph || true

	rm -rf datasets/vendored/cerebro-bot/.git
	rm -rf datasets/vendored/sandbox_agent/.git
	rm -rf datasets/vendored/retrieval-agent-template/.git
	rm -rf datasets/vendored/rag-research-agent-template/.git
	rm -rf datasets/vendored/memory-template/.git
	rm -rf datasets/vendored/react-agent/.git
	rm -rf datasets/vendored/chat-langchain/.git
...
download-helldivers2skulllogo:
	...
	uv run bbid.py -o "datasets/images/photos_of_helldivers2skulllogo" --filters '+filterui:imagesize-large&form=IRFLTR&first=1' "helldivers 2 skull logo" --limit 300

dvc-remote:
	uv run dvc remote add -f -d deeplearning ssh://pi@192.168.3.88:22
